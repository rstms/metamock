...
if __name__ == "__main__":
    ...
    _opt(
        "-p", "--plugin", action="append", help="install additional plugin/s."
    )
...
class Bottle(object):
    ...
    def _cast(self, out, peek=None):
        ...
        if isinstance(out, (tuple, list)) and isinstance(
            out[0], (bytes, unicode)
        ):
...
class BaseResponse(object):
    ...
    def charset(self, default="UTF-8"):
        ...
        if "charset=" in self.content_type:
            return (
                self.content_type.split("charset=")[-1].split(";")[0].strip()
            )
        return default
...
def _lscmp(a, b):
    ...
    return not sum(0 if x == y else 1 for x, y in zip(a, b)) and len(a) == len(
        b
    )
...
class BaseTemplate(object):
    ...
    def __init__(
        ...
        self.filename = (
            source.filename if hasattr(source, "filename") else None
        )
    ...
    def search(cls, name, lookup=[]):
        ...
        if not lookup:
            depr(
                "The template lookup path list should not be empty.", True
            )  # 0.12
            lookup = ["."]
